...
export const NuiProvider = ({
    ...
    const eventListener = (receivedEvent: { data: { app: string; event: string; data: unknown } }) => {
        const { app, event, data } = receivedEvent.data;

        if (app && event) {
            window.dispatchEvent(
                new MessageEvent(eventNameFactory(app, event), {
                    data,
                })
            );
        }
